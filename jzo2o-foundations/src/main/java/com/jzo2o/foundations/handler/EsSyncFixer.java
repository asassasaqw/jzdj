...
import com.jzo2o.foundations.service.impl.ServeSyncServiceImpl;
...
public class EsSyncFixer {
    ...
    @Autowired
    private ServeSyncServiceImpl serveSyncService;

    /**
     * 对数据库和ES中数据进行一致性修复
     */
    @XxlJob(value = "EsSyncFixer")
    public void fix() {
        ...
        if (CollUtil.isNotEmpty(collect)){
            ...
            for (Long id : collect) {
                // 删除请求：指定 索引名 + 文档ID
                DeleteRequest deleteRequest = new DeleteRequest("serve_aggregation", String.valueOf(id));
        ...
        if (CollUtil.isNotEmpty(collect1)){
            ...
            for (Long id : collect1) {
                ...
                IndexRequest indexRequest = new IndexRequest("serve_aggregation", String.valueOf(id));
                // 获取数据库中serve_sync数据
                ServeSync serveSync = serveSyncService.getById(id);
                if (serveSync == null){
                    continue;
                }
                // 封装成ES文档
    ...
    public List<Long> getAllIdsFromDb(){
        return serveSyncService.lambdaQuery()
                .select(ServeSync::getId) // 只查ID字段
                .list()                   // 返回 List<ServeSync>
                .stream()
                .map(ServeSync::getId)    // 抽取 ID → 变成 List<Long>
    ...
    public List<Long> getAllIdsFromEs(){
        //1.封装请求参数
        SearchRequest serveAggregation = new SearchRequest("serve_aggregation");
