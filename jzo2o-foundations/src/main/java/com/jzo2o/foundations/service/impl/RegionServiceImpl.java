...
public class RegionServiceImpl extends ServiceImpl<RegionMapper, Region> implements IRegionService {
    ...
    public void active(Long id) {
        ...
        List<Serve> serves = serveMapper.queryByRegionIdAndSaleStatus(id, FoundationStatusEnum.ENABLE.getStatus());
        if (CollUtils.isEmpty(serves)) {
            throw new ForbiddenOperationException("该区域下不存在上架的服务");
        }
    ...
    public void deactivate(Long id) {
        ...
        //如果禁用区域下有上架的服务则无法禁用
        List<Serve> serves = serveMapper.queryByRegionIdAndSaleStatus(id, FoundationStatusEnum.ENABLE.getStatus());
        if(CollUtils.isNotEmpty(serves)){
            throw new ForbiddenOperationException("该区域下有上架的服务");
        }
        //更新禁用状态
        LambdaUpdateWrapper<Region> updateWrapper = Wrappers.<Region>lambdaUpdate()
                .eq(Region::getId, id)
